...
def register_routes(app):    
    ...
    def download():
        ...
        if not track_set:
            return "upload cannot be parsed or was not done properly", 400
        session["albums_to_remove"] = []
            ...
            while (total > 0):
                ...
                for item in items:
                    ...
                    if (flag):
                        ...
                        session["albums_to_remove"].append(album_id)
    ...
    def done():
        ...
        if not (session.get("uploaded")):
            return "Need upload for data", 400

        if not (session.get("downloaded")):
            return "Need to download before removing!", 400
        ...
        albums = session.get("albums_to_remove")
        ...
        while albums_num > 0:
            ...
            while (i < i_start + 40) and (i < len(albums)):
                request_field += f"spotify%3Aalbum%3A{albums[i]}%2C"
                i += 1
            
            albums_num -= 40
            request_field = request_field[:-3]

            print(request_field)
            response = requests.delete(
                "https://api.spotify.com/v1/me/library?uris=" + request_field,
                headers={"Authorization": f"Bearer {access_token}"}
            )
    ...
    def ask():
        ...
        if (session.get("uploaded")):
            if not session.get("account_id"):
                ...
                account_id = call_result["account_id"]
                session["account_id"] = account_id
            if (retrieve_set_from_cache(session.get("account_id"))):
